...
// --- Razorpay webhook (raw body for webhook signature) ---
app.use("/api/payments/razorpay-webhook", express.raw({ type: "application/json" }));
...
// --- CORS: Allow all origins for API/mobiles ---
app.use(cors());
app.options("*", cors());
...
app.post("/analyze-skills", async (req, res) => {
  try {
    const resp = await axios.post(
      `${process.env.FASTAPI_BASE_URL}/analyze-skills`,
      req.body,
      { timeout: 30000 }
    );
    res.json(resp.data);
...
// ------------------- (Optional) Production static for web only -------------------
// if (process.env.NODE_ENV === "production") {
//   app.use(express.static(path.join(__dirname, "client/build")));
//   app.get("*", (req, res) => {
//     res.sendFile(path.join(__dirname, "client/build/index.html"));
